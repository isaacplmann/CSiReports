...
Name = session("sessionname")
	If (MSO=-1) then 'use mso survey
            If (ParentID=77) or (Name="Gerber")  then
			 	Response.Write "<a href=""javascript: openZoom2('http://www.csicomplete.com/reports_mso/survey_gerber.htm','Survey');""><img src =""http://www.csicomplete.com/images/surimg2.jpg"" border =0><font style=""FONT-FAMILY: sans-serif"" fontsize=2><b>Survey</b></font></a>" 
			Else
				Response.Write "<a href=""javascript: openZoom2('http://www.csicomplete.com/reports_mso/survey_mso.htm','Survey');""><img src =""http://www.csicomplete.com/images/surimg2.jpg"" border =0><font style=""FONT-FAMILY: sans-serif"" fontsize=2><b>Survey</b></font></a>" 
			End If				
    Else
...
select case session("clienttype")
	...
	case "S"
		...
		<%
			strReport = "select ReportName, ReportMonth from tblCollisionReports where (CollisionID=" & session("id") & ") and (active <>'0') and (CollisionReportID=1 or CollisionReportID=15) order by ReportMonth desc"
		...
		%>
			<font face=Arial color=black size=1.5><strong>
		<%
			strReport = "select ReportName, ReportMonth from tblCollisionReports where (CollisionID=" & session("id") & ") and (active <>'0') and (CollisionReportID=2 or CollisionReportID=16) order by ReportMonth desc"
			set rsReport = server.CreateObject("adodb.recordset")
			rsreport.Open strreport,cnlist
