...
varSort = UCase(Request("Sort"))

Response.Write "varsort: " & varsort & "<br>"

select case varSort
	case "STATUS"
		Response.Write "status"
		Response.Write "session(shopid)" & session("shopid") & "<br>"
		Response.Write "session(drptmonth)" & session("drptmonth") & "<br>"
		strDetail = "select * from tblCollisionDetail where ShopID=" & session("shopid") & " and RptMonth='" & session("dRptMonth") & "' order by statusid, CSi desc, lastname"
	case "RO"
		Response.Write "RO"
		Response.Write "session(shopid)" & session("shopid") & "<br>"
		Response.Write "session(drptmonth)" & session("drptmonth") & "<br>"
		strDetail = "select * from tblCollisionDetail where ShopID=" & session("shopid") & " and RptMonth='" & session("dRptMonth") & "' order by RO"
	case else
		Response.Write "else"
		Response.Write "ishopid before reuqestquerystring" & ishopid & "<br>"
		ishopid=Request.QueryString("shopid")
		
		Response.Write "ishopid before session:" & ishopid & "<br>"
		
		session("shopid")=Request.QueryString("shopid")
		Response.Write "ishopid after session:" & session("Shopid") & "<br>"


		dRptMonth=Request.QueryString("Month")
		Response.Write "dRptMonth before session:" & dRptMonth & "<br>"
		session("dRptMonth")=Request.QueryString("Month")
		Response.Write "session(dRptMonth) after session:" & session("dRptMonth") & "<br>"
...
%>
<form name="frmROList"	id=frmEnter method="POST" action="ROList.asp">
<table border="0" width="100%">
    <tr>
        <th align="left"><b><input type="submit" name="Sort" value="Status"></b></th>
        <th align="center"><input type="submit" name="Sort" value="RO"></th>
		<TH align=left>View Survey</TH>
        <th align="left">Last Name</th>
        <th align="left">First Name</th>
        <th align="left">CSi</th>
        <th align="left">Hot Sheet</th>
        <th align="left">Estimator</th>
        <th align="left">Last Call</th>
