...
'strDetail = "select TrendMonth, ROs, QualityQ, Comm, OnTime, ReturnVisit, (avg(RecShopNum)) As RecShopNum " & _
'		" from tblCollisionTrend where (ShopID=" & ishopid & ") and TrendMonth between '" & dateadd("M",-11,MaxMonth) & "' and '" & MaxMonth & "'" & _
'		" group by TrendMonth, ROs, QualityQ, Comm, OnTime, ReturnVisit, RecShop " & _
...
'get the 12M number now....	
	strDetail = "select sum(ROs) as Records, (sum(ros*QualityQ))/(sum(ros)) as QualityQ, " & _
