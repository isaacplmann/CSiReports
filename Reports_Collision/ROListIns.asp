...
strDetail = "select * from tblCollisionDetail where ShopID=" & ishopid & " and RptMonth='" & dRptMonth & "' order by "

strsort = ucase(strsort)
select case strSort
	case "STATUS"
		if session("isort")=1 then
			strDetail = strdetail & "statusid, lastname"
		else
			strDetail = strdetail & "statusid desc, lastname"		
		end if
	case "RO"
		if session("isort")=1 then
	...
	case "QUALITY"
		if session("isort")=1 then
			strDetail = strdetail & "QualityQ desc"
		else
			strDetail = strdetail & "QualityQ"		
		end if
	case "INSURANCE"
		if session("isort")=1 then
			strDetail = strdetail & "InsuranceQ desc"
		else
			strDetail = strdetail & "InsuranceQ"		
		end if
	case "HS"
		if session("isort")=1 then
	...
	response.write "<TABLE width=""100%"">"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=Status'>Status</a></TH>"
	response.write "<TH align=center><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=RO'>RO</a></TH>"
	response.write "<TH align=left><font size=2>View</TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=Last'>Last</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=First'>First</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=ReferShop'>ReferShop</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=Insurance'>Insurance</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=Quality'>Quality</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=HS'>Hot Sheet</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=SW'>Estimator</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=LastCall'>Last Connected Call</a></TH>"
	response.write "<TH align=left><font size=2><a href='ROListNew.asp?ShopID=" & ishopid & "&Month=" & drptmonth & "&Sort=Ins'>Insurer</a></TH>"
	response.write "</tr>"
	...
	If (ParentID = 77 or Name = "Gerber" or MSO=-1) then 'use Gerber
	    If ParentID=8 then
			...
			strURL = "http://www.csicomplete.com/reports_collision/detailpagemsoins.asp?jobid=" & rsdetail.fields("jobid") & "&ShopID=" & ishopid & " "
			case 2
	...
	If IsNumeric(rsdetail.fields("InsuranceQ")) then 
		response.write "<td><font size=2>" & round(rsdetail.fields("InsuranceQ"),1) & "</td>"
	Else
		response.write "<td><font size=2></td>"
	End If
	If IsNumeric(rsdetail.fields("QualityQ")) then 
		response.write "<td><font size=2>" & round(rsdetail.fields("QualityQ"),1) & "</td>"
	Else
		response.write "<td><font size=2></td>"
	End If
