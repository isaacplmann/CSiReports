...
strDetail="select * from (select tblCollisionLogons.Name, tblCollisionTrend.ShopID, ROs as Records, " & _
	"Body, Paint, Mech, Detail, Comm, OnTime, ReturnVisit, " & _
...
strDetail = strdetail & "(select tblCollisionLogons.Name as Name12m, tblCollisionTrend.ShopID as ShopID12m, sum(ROs) as Records12m, " & _
	" (sum(ros*body))/(sum(ros)) as Body12m, " & _
...
if rsdetail.eof then
	...
	do until rsdetail.eof
		Response.write "<tr>"
		response.write "<td colspan=7><font size=2 color=blue><B>" & rsdetail.fields("Name") & "</b></td>"
		response.write "<td colspan=7 align=right><font size=2 color=blue><B>" & rsdetail.fields("Name") & "</b></td>"
		Response.Write "</tr>"
		Response.write "<tr>"
		...
		response.write "<td align=center><font size=2 color=red><B>" & formatnumber(rsdetail.fields("RecShopNum"),2) & "</b></td>"
		...
		response.write "<td align=center><font size=2 color=red><B>" & formatnumber(rsdetail.fields("RecShopNum12m"),2) & "</b></td>"
